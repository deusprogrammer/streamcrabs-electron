...
export default class MediaPoolConfig extends React.Component {
    ...
    loadMediaData = async () => {
        let {videoPool, audioPool} = await window.api.send("getBotConfig");
    ...
    onDisableMedia = async (e, type, index) => {
        ...
        try {
            if (type === "audio") {
                ...
                await window.api.send("updateAudioPool", mediaPool);
            } else if (type === "video") {
                this.setState({videoPool: mediaPool, saving: true});
                await window.api.send("updateVideoPool", mediaPool);
            } else {
                return;
            }
    ...
    storeMedia = async (type) => {
        ...
        if (!this.state.addAudioUrl && !this.state.addVideoUrl) {
            try {
                let url = await window.api.send("storeMedia", mediaData);
                mediaPool.push({
                    id: Date.now(),
                    enabled: false,
            ...
            <div>
                <div style={{display: "table"}}>
                    <div style={{display: "table-cell"}}>
                        ...
                        <ul>
                            { this.state.audioPool.map((element, index) => {
                                ...
                                return (
                                    <li style={{listStyle: index === this.state.selectedAudioIndex ? "disc" : "none"}}>
                    ...
                    <div style={{display: "table-cell", visibility: this.state.selectedAudioIndex >= 0 && this.state.selectedAudioIndex < this.state.audioPool.length ? "visible" : "hidden", verticalAlign: "middle"}}>
                        ...
                        <audio 
                            id="audio-preview"
                            src={createAbsoluteUrl(this.state.audioPreview)} 
                            width="300px"
                            autoPlay
                            controls
                            onEnded={() => {this.setState({selectedAudioIndex: -1})}}
                ...
                <div style={{display: "table"}}>
                    <div style={{display: "table-cell"}}>
                        ...
                        <ul>
                            { this.state.videoPool.map((element, index) => {
                                ...
                                return (<li style={{listStyle: index === this.state.selectedVideoIndex ? "disc" : "none"}}>
                    ...
                    <div style={{display: "table-cell", visibility: this.state.selectedVideoIndex >= 0 && this.state.selectedVideoIndex < this.state.videoPool.length ? "visible" : "hidden", verticalAlign: "middle"}}>
                        ...
                        <video 
                            id="video-preview"
                            src={createAbsoluteUrl(this.state.videoPreview)} 
                            width="300px" 
                            autoPlay
                            onEnded={() => {this.setState({selectedVideoIndex: -1})}}
                            onBlur={() => {this.saveMediaConfig("video")}} />
                    </div>
                </div>
                <div style={{border: "1px solid black"}}>
